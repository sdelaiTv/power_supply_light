// настройки ленты
#define M_WIDTH 9      // ширина матрицы
#define M_HEIGHT 3      // высота матрицы
...
int HUE_START = 100;     // начальный цвет огня (0 красный, 80 зелёный, 140 молния, 190 розовый)
#define HUE_COEF 0.7    // коэффициент цвета огня (чем больше - тем дальше заброс по цвету)
#define SMOOTH_K 0.3   // коэффициент плавности огня
#define MIN_BRIGHT 160   // мин. яркость огня
...
microLED matrix(leds, LED_PIN, M_WIDTH, M_HEIGHT, PARALLEL, RIGHT_TOP, DIR_LEFT);  // объект матрица
...
#define R_RESIST 100000 // сопротивление последовательного резистора, 102 кОм
#define T_RESIST 100000 // номинальное сопротивления термистора, 100 кОм
...
void loop() {
  ...
  HUE_START = map(currentTemp, 35, 48, 140, 0);
  if (HUE_START < 0) HUE_START = 0;
  if (HUE_START > 140) HUE_START = 140;
  Serial.print("TEMP - "); Serial.print(currentTemp); Serial.print("  HUE - "); Serial.println(HUE_START);
  fireRoutine();
  delay(100);
}
